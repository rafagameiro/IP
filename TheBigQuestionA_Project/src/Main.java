...
public class Main {
	 ...
	 * @param args
	 */
	public static final String questions = "questions.txt";
	...
	private static void play(TheBigQuestion theBigQuestion, String name, Scanner in) throws FileNotFoundException{
		...
		if(ans.equalsIgnoreCase("H")){
			if(theBigQuestion.canGetHelp()){
				theBigQuestion.help();
				System.out.println(name + ", " + name + "... Era mesmo facil. Resposta "+ getRightAns(theBigQuestion, qNumber) + " Deseja continuar? (S/N)");
				System.out.print(">");
				ans = in.nextLine();
			...
			if(theBigQuestion.isAnswerRight(qNumber, ans)){
				...
				theBigQuestion.lost();
				System.out.println("Santa ignorancia, "+ name +"! Ficou com " + theBigQuestion.getTotalPrize() + " euros.");
				printScoreBoard(theBigQuestion,name);
				theBigQuestion.nextQuestion();
			}
		}
	...
	private static void tudoOuNada(TheBigQuestion theBigQuestion, String name, Scanner in){
		...
		if(theBigQuestion.isAnswerRight(qNumber, ans)){
			theBigQuestion.resetHelps();
			System.out.println("Parabens, " + name + "! Ficou com " + theBigQuestion.getTotalPrize() + " euros.");
			printScoreBoard(theBigQuestion,name);
			theBigQuestion.nextQuestion();
		}else{
			theBigQuestion.lost();
	...
	private static void printScoreBoard(TheBigQuestion theBigQuestion, String name){
		...
		System.out.println("Parabens, " + name + "." + " Passou a ocupar a posicao " + theBigQuestion.getScoreBoardPosition(name) + " na nossa tabela de recordes.");
		...
		while(theBigQuestion.hasNext()){
			...
			System.out.println(i + ". " + theBigQuestion.getNameIterator() + " " + theBigQuestion.getScoreIterator());
			theBigQuestion.next();
		}
	}
