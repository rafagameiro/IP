...
public class Calculator {
	...
	public double calculate(String exp){
		...
		if(isBinary(op)){
			...
			int end = Brackets(exp);
			num = exp.substring(start + 1, end - 1).trim();
			value1 = calculate(num);
			num2 = exp.substring(end + 1, exp.lastIndexOf(")")).trim();
			value2 = calculate(num2);
			result = binaryFunction(op, value1, value2);
			
		   }else if(isUnary(op)){
		    	double value1 = 0;
		    	num = exp.substring(exp.indexOf("(") + 1, Brackets(exp) - 1).trim();	
		    	value1 = calculate(num);
	...
	/**
	 * This method is used to calculate the position of brackets and when they "open" and "close";
	 * 
	 * @param exp
	 * It is an expression inserted by user;
	 * @return
	 * Returns the position of a bracket;
	 */
	public static int Brackets(String exp){
		int counter = 1;
		int pos = exp.indexOf("(") + 1;
		while(counter != 0 && pos < exp.length()){
			if(exp.charAt(pos) == '('){
				counter += 1;
